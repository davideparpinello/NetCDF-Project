...
int main(int argc, char *argv[])
{
    ...
    while ((de = readdir(dr)) != NULL)
        if (de->d_type == DT_REG)
        {
            ...
            filesList[counter] = (char*) malloc (strlen(name)+1);
    ...
    double **medie_finali[file_count];
    for (counter = 0; counter < file_count; counter++)
    {
        ...
        double elapsed_time_lettura=0;
        double elapsed_time_matrice=0;
        ...
        if (rank == 0)
        {
            printf("File elaborato: %s (counter: %d) \n", filename, counter);
        ...
        */
        ...
        if (rank == 0)
        {
            ...
            medie_finali[counter] = (double**)somma_finale;
